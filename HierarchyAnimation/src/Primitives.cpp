...
void Joint::draw() {
	...
	if (parent) {
		...
		glm::vec3 midPoint = (parentPos + thisPos) * 0.5f;

		glm::vec3 direction = glm::normalize(parentPos - thisPos);
		float length = glm::distance(parentPos, thisPos);
		glm::mat4 rotationMatrix = rotateToVector(glm::vec3(0,1,0), direction);

		ofPushMatrix();
		ofTranslate(midPoint);
		ofMultMatrix(rotationMatrix);
		ofDrawCylinder(0.2f, length);
		ofPopMatrix();
	}
}
