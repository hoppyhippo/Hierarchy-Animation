				...
				if (joint && joint->keyFrames.size() > 1) {
					for (size_t i = 0; i < joint->keyFrames.size() - 1; i++) {
						...
						if (frame >= kf1.frame && frame <= kf2.frame) {
							joint->position = 0.5 * linearInterp(frame, kf1.frame, kf2.frame, kf1.position, kf2.position) +
								0.5 * easeInterp(frame, kf1.frame, kf2.frame, kf1.position, kf2.position);
							joint->rotation = 0.5 * linearInterp(frame, kf1.frame, kf2.frame, kf1.rotation, kf2.rotation) +
								0.5 * easeInterp(frame, kf1.frame, kf2.frame, kf1.rotation, kf2.rotation);
							joint->scale = 0.5 * linearInterp(frame, kf1.frame, kf2.frame, kf1.scale, kf2.scale) +
								0.5 * easeInterp(frame, kf1.frame, kf2.frame, kf1.scale, kf2.scale);
							break;
						}
					}
				}
